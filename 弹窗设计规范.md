...
**当前方式：blankView 和 popupView 是同级关系**

```objc
// 库内部：分别添加到 superview
[popupSuperview addSubview:blankView];   // 遮罩层
[popupSuperview addSubview:popupView];   // 弹窗内容
```

**问题：**
- 两个视图分别添加，库需要分别管理
- 动画时需要分别处理 blankView 和 popupView
...
- (instancetype)initWithPopupView:(UIView *)popupView
                   ...
                   tapBlankHandle:(void(^)(CJCenterBlankView *bSelf))tapBlankHandle;
@end

// 使用方式
CJBottomBlankView *bottomView = [[CJBottomBlankView alloc] initWithPopupView:popupView
                                                            popupViewHeight:200
                                                             tapBlankHandle:^(CJBottomBlankView *bSelf) {
                                                                 // tap action
                                                             }];
[popupSuperview addSubview:bottomView];
```

- **优势**：基类强制实现协议，编译时检查，防止子类忘记实现 `updateConstraintsForPopupViewWithShow:`
- **问题**：每种弹窗要记不同的类名（CJBottomBlankView、CJCenterBlankView 等）

#### 过渡：动画优化

**当前动画方式**：库内部调用 UIView 动画方法，分别处理 blankView 和 popupView

**优化后**：blankView 内部通过 `updateConstraintsForPopupViewWithShow:` 统一处理布局和动画

```objc
// blankView 内部实现
- (void)showBlankViewSelfInView:(UIView *)superview {
    [superview addSubview:self];
    [self updateConstraintsForPopupViewWithShow:YES];
}
...
**优势**：动画逻辑封装在 blankView 内部，库不需要单独处理动画
...
[bottomView showBlankViewSelfInView:popupSuperview];
```

- **优势**：`showBlankViewSelfInView:` 是标准方法，内部自动处理布局和动画，不需要手动调用 addSubview
...
+ (void)cj_expandAnimateView:(UIView *)animatedView
    ...
    // 所有参数都是硬编码
    if (blankView != nil) {
        blankView.alpha = forShow ? 0.2 : 1.0;  // 0.2 写死
    }
    animatedView.frame = forShow ? hideFrame : showFrame;
    animatedView.alpha = forShow ? 0.2 : 1.0;  // 0.2 写死
    [UIView animateWithDuration:0.3 animations:^{  // 0.3s 写死
        if (blankView != nil) {
            blankView.alpha = forShow ? 1.0 : 0.0;
        }
        animatedView.alpha = forShow ? 1.0 : 0.0;
        animatedView.frame = forShow ? showFrame : hideFrame;
...
**问题：**
- 产品说"所有弹窗遮罩透明度从 0.2 改为 0.3" → 要改所有调用点
- 需要给所有动画添加统计埋点 → 要改所有调用点
- 全局替换动画曲线 → 要改所有调用点

**解决方案：** 使用拦截器设计，通过 Block 拦截底层动画执行，参考 [拦截器设计规范](拦截器设计规范.md)。
